...
Simple Approach : Brute Force
    ...
    2) starting from every element,make subarrays using 2 nested for loops
    3) for 1st loop, i=0 to i=n-1
    4) Initialize curr_sum = 0
    5) for inner for loop,j=i to j=n-1
    6) add A[j] to curr_sum for every pass.
...
Best Approach : Kadane's Algorithm with Time Complexity = O(n)
